...
describe('map visibility selector', () => {
  ...
  it('keeps legacy storage compatibility while exposing separate proposed plan visibility', () => {
    ...
    assert.match(visibility, /poiTypes/);
    assert.match(visibility, /planCategoryValues/);
    assert.match(visibility, /tripPoiTypeValues/);
    ...
    assert.match(visibility, /map-tool-accordion/);
    ...
    assert.match(tripMap, /visibility\.poiTypes\[point\.type\]/);
